...
import type { Field, Tournament, AdminStats } from '@/types'

export function AdminPanelPage() {
  const [tab, setTab] = useState<'stats' | 'fields' | 'tournaments'>('stats')
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24">
      ...
      <div className="flex gap-1 mx-4 p-1 bg-surface rounded-2xl mb-4">
        {(['stats', 'fields', 'tournaments'] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
          ...
          >
            {t === 'stats' ? 'Обзор' : t === 'fields' ? 'Поля' : 'Турниры'}
